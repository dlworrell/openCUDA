...
def scan_repository(root: Path, policy: dict[str, Any]) -> list[Violation]:
    ...
    for path in root.rglob("*"):
        ...
        generated_dirs = policy["forbidden_generated_directories"]
        if any(part in generated_dirs for part in parts):
            violations.append(
                Violation(
                    ...
                    relative,
                    1,
                    "generated/cache directory committed",
                )
            )
        ...
        if path.suffix.lower() in forbidden_exts:
            violations.append(
                Violation(
                    ...
                    relative,
                    1,
                    "compiled/archive artifact must not be committed",
                )
            )
        ...
        if size > max_size:
            violations.append(
                Violation(
                    ...
                    relative,
                    1,
                    f"file exceeds {max_size} byte repository limit",
                )
            )

        should_scan_text = path.suffix in text_exts or path.name in {
            "CMakeLists.txt",
            "Makefile",
        }
        ...
        try:
            ...
            violations.append(
                Violation("encoding", relative, 1, "text file is not valid UTF-8")
            )
            continue
        ...
        if text and not text.endswith("\n"):
            violations.append(
                Violation(
                    ...
                    relative,
                    text.count("\n") + 1,
                    "text file must end with newline",
                )
            )
        for index, line_text in enumerate(text.splitlines(), start=1):
            ...
            unexpected_control = any(
                unicodedata.category(ch) == "Cf" and ch not in {"\u200c", "\u200d"}
                for ch in line_text
            )
            if unexpected_control:
                violations.append(
                    Violation(
                        ...
                        relative,
                        index,
                        "unexpected Unicode format/control character",
                    )
                )
            ...
            if match:
                violations.append(
                    Violation(
                        ...
                        "probable secret/private key material",
                    )
                )
        ...
        if top in scan_dirs and _source_file(path):
                ...
                if match:
                    violations.append(
                        Violation(
                            ...
                            f"unsafe C API {name}() is prohibited",
                        )
                    )
            for name in policy["discouraged_c_apis"]:
                ...
                if match:
                    violations.append(
                        Violation(
                            f"discouraged-api:{name}",
                            relative,
                            _line(code, match.start()),
                            (
                                f"legacy bounded string API {name}() requires "
        ...
        if relative.startswith("include/opencuda/"):
            for type_name in policy["public_abi_forbidden_types"]:
                ...
                if match:
                    violations.append(
                        Violation(
                            "public-abi-cuda-type",
                            relative,
                            _line(text, match.start()),
                            f"raw CUDA type {type_name} leaks into the public ABI",
                        )
                    )
