...
def test_every_block_term_is_detected_without_duplicating_fixture_vocabulary() -> None:
    ...
    for category in policy["categories"]:
        ...
        for term in category["terms"]:
            ...
            assert any(
                item.category == category["id"] and item.severity == "block"
...
def test_configured_adult_link_rule_is_detected() -> None:
    ...
    findings = scan_text(
        f"https://example-{token}.invalid/resource",
        policy,
        include_media=False,
    )
...
def test_embedded_media_requires_review() -> None:
    policy = load_policy(ROOT / "config" / "content-policy.json")
    findings = scan_text(
        "![benchmark plot](attachment.png)",
        policy,
        include_media=True,
    )
    assert any(
        item.rule_id == policy["media_review"]["id"] and item.severity == "review"
        for item in findings
    )


def test_policy_file_is_valid_json() -> None:
    policy_path = ROOT / "config" / "content-policy.json"
